...
// RAII wrapper for OpenSSL's EVP_MD_CTX to ensure proper resource management
class DigestContext {
private:
    EVP_MD_CTX* ctx;

public:
    DigestContext() : ctx(EVP_MD_CTX_new()) {
    ...
    // Accessor for the underlying OpenSSL context
    EVP_MD_CTX* get() { 
        return ctx; 
    }
    ...
    // Prevent moving
    DigestContext(DigestContext&& other) = delete;
    DigestContext &operator=(DigestContext &&other) = delete;
};
...
std::string computeSHA256FromDataBuffer(const uint8_t* data, size_t size) {
    DigestContext ctx;  // Create a new message digest context
